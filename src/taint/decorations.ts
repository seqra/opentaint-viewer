...
export interface StepDecoration {
  ...
  wholeLine: boolean;
  /** Red for the sink (last step), strong for the current step, faint for the rest. */
  className: string;
 ...
 * The sink (last step) is always red; the current step is strong (with a gutter
 * arrow); the rest of the path is faint.
 */
export function pathDecorations(steps: TaintStep[], file: string, currentIndex: number): StepDecoration[] {
  ...
  return steps
    ...
    .map((s) => {
      ...
      const isSink = s.index === lastIndex;
      ...
      return {
        ...
        className: isSink ? 'taint-sink' : isCurrent ? 'taint-current' : 'taint-faint',
