      ...
      <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-2)', borderBottom: '1px solid var(--border)', fontSize: 11, fontFamily: 'var(--mono)' }}>
        ...
        {finding && (
          <div data-testid="step-nav" style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '2px 6px' }}>
            ...
            <button type="button" title="Step back (←)" disabled={atStart} onClick={() => step('back')} style={navBtn}>◀</button>
            <button type="button" title="Step in (→)" disabled={atEnd} onClick={() => step('in')} style={navBtn}>⤵</button>
            <button type="button" title="Step over (Shift+→)" disabled={atEnd} onClick={() => step('over')} style={navBtn}>⤳</button>
            <button type="button" title="Step out (Shift+←)" disabled={atEnd} onClick={() => step('out')} style={navBtn}>⤴</button>
